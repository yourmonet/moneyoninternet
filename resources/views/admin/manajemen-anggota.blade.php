...
<title>Manajemen Mahasiswa - Admin - {{ app_setting('app_name', 'MONET') }}</title>
...
<main class="md:ml-64 p-4 pt-20 md:p-8 md:pt-20 min-h-screen">
    <header class="flex justify-between items-end mb-10">
        <div>
            <h1 class="text-4xl font-headline font-extrabold tracking-tight text-on-surface">Manajemen Mahasiswa</h1>
            <p class="text-on-surface-variant font-body mt-1">Daftar mahasiswa dan pembuatan akun mahasiswa baru.</p>
        </div>
    </header>
    ...
    <div class="bg-surface-container-lowest rounded-3xl shadow-sm border border-outline-variant/30 overflow-hidden">
        <div class="p-6 border-b border-outline-variant/30 flex flex-col sm:flex-row justify-between items-center gap-4 bg-surface-container-low/50">
            <h2 class="text-lg font-bold text-on-surface shrink-0">Daftar Mahasiswa</h2>
            
            <form action="{{ route('admin.anggota.index') }}" method="GET" class="w-full sm:max-w-md relative flex-1 flex group">
                ...
                <input type="text" name="search" value="{{ $search ?? '' }}" placeholder="Cari mahasiswa..." class="w-full pl-10 pr-12 py-2 text-sm rounded-xl border border-outline-variant bg-surface-container-lowest focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all outline-none">
                        ...
                        <tr class="hover:bg-surface-container/30 transition-colors">
                            <td class="px-6 py-4">
                                <div class="flex items-center gap-3">
                                    ...
                                    @endif
                                    <div>
                                        <p class="font-bold text-on-surface">{{ $a->name }}</p>
                                        <p class="text-xs text-on-surface-variant">{{ $a->email }}</p>
                                    </div>
                                </div>
                            </td>
                            <td class="px-6 py-4">
                            ...
                            <td class="px-6 py-4 text-right">
                                <div class="flex items-center justify-end gap-2">
                                    <form id="delete-form-{{ $a->id }}" action="{{ route('admin.anggota.destroy', $a->id) }}" method="POST">
                                        @csrf
                                        @method('DELETE')
                                        <button type="button" onclick="confirmDelete('delete-form-{{ $a->id }}')" class="p-2 rounded-xl text-error hover:bg-error-container/40 transition-colors tooltip-trigger" title="Hapus Akun">
                    ...
                    @empty
                        <tr>
                            <td colspan="3" class="px-6 py-12 text-center text-on-surface-variant">
                                ...
                                <p>Belum ada mahasiswa aktif. Silakan tambahkan akun mahasiswa.</p>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
...
<div id="add-modal" class="fixed inset-0 z-[100] flex items-center justify-center hidden opacity-0 transition-opacity duration-300">
    ...
    <div class="bg-surface-container-lowest p-8 rounded-3xl shadow-2xl w-full max-w-lg relative z-10 transform scale-95 transition-transform duration-300 mx-4">
        <h2 class="text-2xl font-headline font-extrabold text-on-surface mb-6">Tambah Akun Mahasiswa</h2>
        
        <form action="{{ route('admin.anggota.store') }}" method="POST" class="space-y-5">
            @csrf
            <div>
                <label for="student_id" class="block text-sm font-medium text-on-surface-variant mb-1">Pilih Mahasiswa</label>
                <select name="student_id" id="student_id" required class="w-full rounded-xl border border-outline-variant bg-surface-container-low px-4 py-3 focus:border-primary focus:ring-2 focus:ring-primary/20 outline-none transition-all">
                    <option value="">-- Pilih Mahasiswa (Nama - NIM) --</option>
