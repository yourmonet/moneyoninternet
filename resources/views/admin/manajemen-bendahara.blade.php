...
<title>Manajemen Bendahara - Admin - {{ app_setting('app_name', 'MONET') }}</title>
...
<main class="md:ml-64 p-4 pt-20 md:p-8 md:pt-20 min-h-screen">
    <header class="flex justify-between items-end mb-10">
        <div>
            <h1 class="text-4xl font-headline font-extrabold tracking-tight text-on-surface">Manajemen Bendahara</h1>
            <p class="text-on-surface-variant font-body mt-1">Daftar bendahara dan pembuatan akun bendahara baru.</p>
        </div>
    </header>
    ...
    <div class="bg-surface-container-lowest rounded-3xl shadow-sm border border-outline-variant/30 overflow-hidden">
        <div class="p-6 border-b border-outline-variant/30 flex flex-col sm:flex-row justify-between items-center gap-4 bg-surface-container-low/50">
            <h2 class="text-lg font-bold text-on-surface shrink-0">Daftar Bendahara</h2>
            
            <form action="{{ route('admin.bendahara.index') }}" method="GET" class="w-full sm:max-w-md relative flex-1 flex group">
                ...
                <input type="text" name="search" value="{{ $search ?? '' }}" placeholder="Cari bendahara..." class="w-full pl-10 pr-12 py-2 text-sm rounded-xl border border-outline-variant bg-surface-container-lowest focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all outline-none">
                        ...
                        <tr class="hover:bg-surface-container/30 transition-colors">
                            <td class="px-6 py-4">
                                <div class="flex items-center gap-3">
                                    @if($bendahara->avatar)
                                        ...
                                        @endphp
                                        <img src="{{ $avatarUrl }}" alt="{{ $bendahara->name }}" class="w-10 h-10 rounded-full object-cover shadow-sm shrink-0" onerror="this.style.display='none'; this.nextElementSibling.style.display='flex';">
                                        <div class="w-10 h-10 rounded-full bg-secondary-container text-on-secondary-container font-bold text-lg shrink-0" style="display:none; align-items:center; justify-content:center;">
                                            {{ strtoupper(substr($bendahara->name, 0, 1)) }}
                                        </div>
                                    @else
                                    ...
                                    @endif
                                    <div>
                                        <p class="font-bold text-on-surface">{{ $bendahara->name }}</p>
                                        <p class="text-xs text-on-surface-variant">{{ $bendahara->email }}</p>
                                    </div>
                                </div>
                            </td>
                            <td class="px-6 py-4">
                            ...
                            <td class="px-6 py-4 text-right">
                                <div class="flex items-center justify-end gap-2">
                                    <form id="delete-form-{{ $bendahara->id }}" action="{{ route('admin.bendahara.destroy', $bendahara->id) }}" method="POST">
                                        @csrf
                                        @method('DELETE')
                                        <button type="button" onclick="confirmDelete('delete-form-{{ $bendahara->id }}')" class="p-2 rounded-xl text-error hover:bg-error-container/40 transition-colors tooltip-trigger" title="Hapus Akun">
                    ...
                    @empty
                        <tr>
                            <td colspan="3" class="px-6 py-12 text-center text-on-surface-variant">
                                ...
                                <p>Belum ada bendahara. Silakan tambahkan akun bendahara.</p>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
...
<div id="add-modal" class="fixed inset-0 z-[100] flex items-center justify-center hidden opacity-0 transition-opacity duration-300">
    ...
    <div class="bg-surface-container-lowest p-8 rounded-3xl shadow-2xl w-full max-w-lg relative z-10 transform scale-95 transition-transform duration-300 mx-4">
        <h2 class="text-2xl font-headline font-extrabold text-on-surface mb-6">Tambah Akun Bendahara</h2>
        
        <form action="{{ route('admin.bendahara.store') }}" method="POST" class="space-y-5">
            @csrf
            <div>
