...
<title>Data Mahasiswa - Admin - {{ app_setting('app_name', 'MONET') }}</title>
...
<main class="md:ml-64 p-4 pt-20 md:p-8 md:pt-20 min-h-screen">
    <header class="flex justify-between items-end mb-10">
        <div>
            <h1 class="text-4xl font-headline font-extrabold tracking-tight text-on-surface">Data Mahasiswa</h1>
            <p class="text-on-surface-variant font-body mt-1">Daftar NIM dan nama mahasiswa yang berhak mendaftar akun.</p>
        </div>
    </header>
    ...
    <div class="bg-surface-container-lowest rounded-3xl shadow-sm border border-outline-variant/30 overflow-hidden">
        <div class="p-6 border-b border-outline-variant/30 flex flex-col sm:flex-row justify-between items-center gap-4 bg-surface-container-low/50">
            <h2 class="text-lg font-bold text-on-surface shrink-0">Daftar Data Mahasiswa</h2>
            
            <form action="{{ route('admin.mahasiswa.index') }}" method="GET" class="w-full sm:max-w-md relative flex-1 flex group">
                ...
                <input type="text" name="search" value="{{ $search ?? '' }}" placeholder="Cari data mahasiswa..." class="w-full pl-10 pr-12 py-2 text-sm rounded-xl border border-outline-variant bg-surface-container-lowest focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all outline-none">
                ...
                <tbody class="divide-y divide-outline-variant/20">
                    @forelse($students as $student)
                        <tr class="hover:bg-surface-container/30 transition-colors">
                            <td class="px-6 py-4 font-bold text-primary">{{ $student->nim }}</td>
                            <td class="px-6 py-4 text-on-surface font-medium">{{ $student->name }}</td>
                            <td class="px-6 py-4 text-on-surface-variant text-sm">{{ $student->angkatan ?? '-' }}</td>
                            <td class="px-6 py-4 text-right">
                                <div class="flex items-center justify-end gap-2">
                                    <button type="button" onclick="showEditModal('{{ $student->id }}', '{{ $student->nim }}', '{{ $student->name }}', '{{ $student->angkatan }}')" class="p-2 rounded-xl text-blue-600 hover:bg-blue-50 transition-colors tooltip-trigger" title="Edit Data">
                                        <span class="material-symbols-outlined text-[20px]">edit</span>
                                    </button>
                                    <form id="delete-form-{{ $student->id }}" action="{{ route('admin.mahasiswa.destroy', $student->id) }}" method="POST">
                                        @csrf
                                        @method('DELETE')
                                        <button type="button" onclick="confirmDelete('delete-form-{{ $student->id }}')" class="p-2 rounded-xl text-error hover:bg-error-container/40 transition-colors tooltip-trigger" title="Hapus Data">
...
<div id="add-modal" class="fixed inset-0 z-[100] flex items-center justify-center hidden opacity-0 transition-opacity duration-300">
    ...
    <div class="bg-surface-container-lowest p-8 rounded-3xl shadow-2xl w-full max-w-lg relative z-10 transform scale-95 transition-transform duration-300 mx-4">
        <h2 class="text-2xl font-headline font-extrabold text-on-surface mb-6">Tambah Data Mahasiswa</h2>
        
        <form action="{{ route('admin.mahasiswa.store') }}" method="POST" class="space-y-5">
            @csrf
            <div>
                <label for="nim" class="block text-sm font-medium text-on-surface-variant mb-1">NIM Mahasiswa</label>
                <input type="text" name="nim" id="nim" required class="w-full rounded-xl border border-outline-variant bg-surface-container-low px-4 py-3 focus:border-primary focus:ring-2 focus:ring-primary/20 outline-none transition-all">
            </div>
            <div>
            ...
            <div>
                <label for="angkatan" class="block text-sm font-medium text-on-surface-variant mb-1">Angkatan</label>
                <input type="text" name="angkatan" id="angkatan" placeholder="Contoh: 2026" class="w-full rounded-xl border border-outline-variant bg-surface-container-low px-4 py-3 focus:border-primary focus:ring-2 focus:ring-primary/20 outline-none transition-all">
            </div>
...
{{-- Edit Modal --}}
<div id="edit-modal" class="fixed inset-0 z-[100] flex items-center justify-center hidden opacity-0 transition-opacity duration-300">
    <div class="absolute inset-0 bg-black/40 backdrop-blur-sm" onclick="hideEditModal()"></div>
    <div class="bg-surface-container-lowest p-8 rounded-3xl shadow-2xl w-full max-w-lg relative z-10 transform scale-95 transition-transform duration-300 mx-4">
        <h2 class="text-2xl font-headline font-extrabold text-on-surface mb-6">Edit Data Mahasiswa</h2>
        
        <form id="edit-form" method="POST" class="space-y-5">
            @csrf
            @method('PUT')
            <div>
                <label for="edit_nim" class="block text-sm font-medium text-on-surface-variant mb-1">NIM Mahasiswa</label>
                <input type="text" name="nim" id="edit_nim" required class="w-full rounded-xl border border-outline-variant bg-surface-container-low px-4 py-3 focus:border-primary focus:ring-2 focus:ring-primary/20 outline-none transition-all">
            </div>
            <div>
                <label for="edit_name" class="block text-sm font-medium text-on-surface-variant mb-1">Nama Lengkap</label>
                <input type="text" name="name" id="edit_name" required class="w-full rounded-xl border border-outline-variant bg-surface-container-low px-4 py-3 focus:border-primary focus:ring-2 focus:ring-primary/20 outline-none transition-all">
            </div>
            <div>
                <label for="edit_angkatan" class="block text-sm font-medium text-on-surface-variant mb-1">Angkatan</label>
                <input type="text" name="angkatan" id="edit_angkatan" placeholder="Contoh: 2026" class="w-full rounded-xl border border-outline-variant bg-surface-container-low px-4 py-3 focus:border-primary focus:ring-2 focus:ring-primary/20 outline-none transition-all">
            </div>
            <div class="flex gap-3 mt-8">
                <button type="button" onclick="hideEditModal()" class="flex-1 py-3 px-4 rounded-xl border border-outline-variant/30 text-on-surface-variant font-bold text-sm hover:bg-surface-container transition-colors">
                    Batal
                </button>
                <button type="submit" class="flex-1 bg-primary text-on-primary font-bold py-3 px-4 rounded-xl shadow-md hover:bg-primary/90 hover:shadow-lg transition-all flex items-center justify-center gap-2 text-sm">
                    <span class="material-symbols-outlined text-lg">save</span>
...
<script>
    ...
    const editModalContent = editModal.querySelector('div.bg-surface-container-lowest');

    function showEditModal(id, nim, name, angkatan) {
        document.getElementById('edit-form').action = '/admin/mahasiswa/' + id;
    ...
    function confirmDelete(formId) {
        Swal.fire({
            title: 'Hapus Data?',
            text: "Data mahasiswa yang bersangkutan akan dihapus!",
            icon: 'warning',
