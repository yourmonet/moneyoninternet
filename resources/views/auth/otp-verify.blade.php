...
<title>Verifikasi Email | {{ app_setting('app_name', 'MONET') }}</title>
...
<main class="w-full max-w-md bg-surface-container-lowest rounded-xl overflow-hidden shadow-[0px_12px_32px_rgba(25,28,30,0.04)] ring-1 ring-outline-variant/10 p-8 md:p-12">
    ...
    <header class="mb-8 text-center">
        <h2 class="font-headline text-2xl font-bold text-on-surface mb-2">Verifikasi Email Anda</h2>
        <p class="text-on-surface-variant text-sm">Masukkan 6 digit kode OTP yang telah kami kirimkan ke email Anda.</p>
    </header>
    ...
    <form id="verify-form" method="POST" action="{{ route('verification.verify.otp') }}" class="space-y-5" onsubmit="showLoading()">
        @csrf
        <div class="space-y-2">
    ...
    <div class="mt-6 text-center">
        <p class="text-sm text-on-surface-variant">
            ...
            <form method="POST" action="{{ route('verification.send.otp') }}" class="inline">
                @csrf
                <button type="submit" class="font-bold text-primary hover:underline underline-offset-4 ml-1">Kirim ulang</button>
            </form>
        </p>
    </div>
</main>
