@forelse($kasMasuk as $km)
    <tr class="hover:bg-surface/50 transition-colors group">
        <td class="px-6 py-4 text-on-surface-variant font-medium whitespace-nowrap">
            {{ \Carbon\Carbon::parse($km->tanggal)->translatedFormat('d F Y') }}
        </td>
        <td class="px-6 py-4">
            <div class="font-bold text-on-surface">{{ $km->keterangan }}</div>
            @if($km->user)
                <div class="text-[10px] uppercase tracking-wider text-outline font-bold mt-1 flex items-center gap-1">
                    <span class="material-symbols-outlined text-xs">person</span>
                    <span>Anggota: {{ $km->user->name }}</span>
                </div>
            @endif
        </td>
