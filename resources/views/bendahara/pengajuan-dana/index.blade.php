...
<link class="icon" type="image/png" href="https://cdn-1.yourmonet.web.id/images/monet.png">
=======
<title>Pengajuan Dana - Bendahara | MONET</title>
>>>>>>> fitur-status-final
...
<main class="ml-64 pt-20 p-8 min-h-screen">
    ...
    <div class="bg-surface-container-lowest rounded-3xl p-6 mb-8 border border-outline-variant/30 shadow-sm flex flex-col md:flex-row md:items-center gap-4 justify-between">
        <form method="GET" action="{{ route('bendahara.pengajuan-dana.index') }}" class="flex flex-col sm:flex-row gap-3 w-full">
            ...
            @if(request()->anyFilled(['search', 'status']))
                <a href="{{ route('bendahara.pengajuan-dana.index') }}" class="inline-flex items-center justify-center border border-outline-variant/30 text-on-surface-variant px-5 py-2.5 rounded-xl text-sm font-bold hover:bg-surface-container transition-colors">
                    Reset
                </a>
            @endif
        </form>
                    ...
                    @forelse ($pengajuans as $item)
                        <tr class="hover:bg-surface-container/30 transition-colors text-sm">
                            ...
                            <td class="py-4 px-4 text-center whitespace-nowrap">
                                <a href="{{ route('bendahara.pengajuan-dana.show', $item->id) }}" class="inline-flex items-center gap-1 bg-primary-container/20 text-primary hover:bg-primary-container/40 px-4 py-2 rounded-xl text-xs font-bold transition-all">
                    ...
                    timelineHtml += `
                        <div class="relative flex gap-3">
                            ...
                            <div>
                                ...
                                <p class="text-[11px] text-outline mt-1 font-semibold">Oleh: ${data.approved_by ? data.approved_by.name : (data.rejected_by ? data.rejected_by.name : 'Bendahara')}</p>
