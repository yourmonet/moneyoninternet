...
<title>Buat Pengajuan Dana — {{ app_setting('app_name', 'MONET') }}</title>
...
<main class="md:ml-64 pt-20 p-4 pt-20 md:p-8 md:pt-20 min-h-screen">
    <header class="flex justify-between items-end mb-10">
        <div>
            <div class="flex items-center gap-2 text-sm text-on-surface-variant font-medium mb-1">
                <a href="{{ route('pengurus.dashboard') }}" class="hover:text-primary transition-colors">Dashboard</a>
                <span class="material-symbols-outlined text-xs">chevron_right</span>
                <a href="{{ route('pengurus.pengajuan-dana.index') }}" class="hover:text-primary transition-colors">Pengajuan Dana</a>
                <span class="material-symbols-outlined text-xs">chevron_right</span>
    ...
    <div class="bg-surface-container-lowest rounded-3xl shadow-sm border border-outline-variant/30 w-full overflow-hidden">
        <form action="{{ route('pengurus.pengajuan-dana.store') }}" method="POST" enctype="multipart/form-data" class="p-8 space-y-6">
            @csrf
            ...
            <div class="flex justify-end gap-3 pt-4 border-t border-outline-variant/20">
                <a href="{{ route('pengurus.pengajuan-dana.index') }}" class="py-3 px-6 rounded-2xl border border-outline-variant/30 text-on-surface-variant font-bold text-sm hover:bg-surface-container transition">
                    Batal
                </a>
